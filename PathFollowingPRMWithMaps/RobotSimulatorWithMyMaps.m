classdef RobotSimulatorWithMyMaps < handle
    %RobotSimulatorWithMyMaps Simulator for simple differential-drive robot
    ...
    %
    %   OBJ = RobotSimulatorWithMyMaps creates a simple simulator for a
    ...
    %
    %   OBJ = RobotSimulatorWithMyMaps(MAPNAME) loads a predefined map
    ...
    %
    %   OBJ = RobotSimulatorWithMyMaps(MAPNAME,MAPRESOLUTION) loads a
    %   predefined map with MAPNAME and sets the resolution to
    %   MAPRESOLUTION (in cells / meter). By default, the resolution is 2
    %   cells / meter.
    %
    %   OBJ = RobotSimulatorWithMyMaps(BOG) loads the map defined by the
    %   robotics.BinaryOccupancyGrid object BOG into the simulator.
    %
    %   Notes:
    %   1) RobotSimulatorWithMyMaps's ROS interface expects MATLAB ROS
    ...
    %
    %   RobotSimulatorWithMyMaps Published Topics:
    ...
    %
    %   RobotSimulatorWithMyMaps Subscribed Topics:
    %       /mobile_base/commands/velocity - Velocity commands to the robot
    %
    %   RobotSimulatorWithMyMaps Services:
    ...
    %   various properties.
    %
    %   RobotSimulatorWithMyMaps properties:
    %
    ...
    %
    %   RobotSimulatorWithMyMaps methods:
    %
    ...
    %
    %     % Create Simulator
    %     robotsim = RobotSimulatorWithMyMaps;
    %
    ...
    properties(Constant, Access = protected)
        ...
        FigureTag = 'RobotSimulatorWithMyMaps'
    end
    ...
    properties (Access = {?RobotSimulatorWithMyMaps, ?matlab.unittest.TestCase})
    ...
    methods
        function obj = RobotSimulatorWithMyMaps(varargin)
            %RobotSimulatorWithMyMaps Constructor for simulator object
            %   Please check the class definition for details.
            
            obj.MyFigureTag = RobotSimulatorWithMyMaps.FigureTag;
            ...
            examplesFilePath = fullfile(fileparts(mfilename('fullpath')), 'Maps.mat');
            ...
            switch nargin
                case 0
                    % RobotSimulatorWithMyMaps() syntax
                    % Use default values
                    mapName = 'emptyMap';
                    mapResolution = 2;
                ...
                case 1
                    % RobotSimulatorWithMyMaps(MAPNAME) or
                    % RobotSimulatorWithMyMaps(BOG) syntax
                    map = varargin{1};
                    if ischar(map)
                        % RobotSimulatorWithMyMaps(MAPNAME) syntax
                        mapResolution = 2;
                        mapName = validatestring(map, fieldnames(exampleMaps), 'RobotSimulatorWithMyMaps', 'mapName');
                        obj.InternalMap = robotics.BinaryOccupancyGrid(exampleMaps.(mapName), mapResolution);
                    else
                        % RobotSimulatorWithMyMaps(BOG) syntax
                        validateattributes(map, {'robotics.BinaryOccupancyGrid'}, {'nonempty','scalar'}, ...
                            'RobotSimulatorWithMyMaps', 'bog');
                        obj.InternalMap = map;
                    end
                case 2
                    % RobotSimulatorWithMyMaps(MAPNAME, MAPRESOLUTION) syntax
                    mapName = varargin{1};
                    mapResolution = varargin{2};
                    mapName = validatestring(mapName, fieldnames(exampleMaps), 'RobotSimulatorWithMyMaps', 'mapName');
                    validateattributes(mapResolution, {'double'}, {'nonempty','scalar'}, 'RobotSimulatorWithMyMaps', 'mapResolution');
                    obj.InternalMap = robotics.BinaryOccupancyGrid(exampleMaps.(mapName), mapResolution);
                otherwise
                    error('RobotSimulator:InvalidInput',...
                        'Invalid number of arguments to RobotSimulatorWithMyMaps.');
            end
        ...
        function setRobotSize(obj, radius)
            ...
            validateattributes(radius, {'numeric'}, {'nonempty', 'scalar', 'positive'}, 'setRobotSize', 'radius');
        ...
        function setRobotPose(obj, pose)
            ...
            validateattributes(pose, {'numeric'}, {'nonempty', 'vector', 'numel', 3}, 'setRobotPose', 'pose');
        ...
        function drive(obj, v, omega)
            ...
            validateattributes(v, {'double'}, {'nonempty', 'scalar'}, 'drive', 'v');
            validateattributes(omega, {'double'}, {'nonempty', 'scalar'}, 'drive', 'omega');
    ...
    methods (Access = private)
        ...
        function plotTrajectory(obj)
            ...
            if isempty(obj.TrajectoryHandle) || ~isgraphics(obj.TrajectoryHandle)
                obj.TrajectoryHandle = plot(obj.Axes, trajectory(:,1), trajectory(:,2),'r','LineWidth',3);
            else
