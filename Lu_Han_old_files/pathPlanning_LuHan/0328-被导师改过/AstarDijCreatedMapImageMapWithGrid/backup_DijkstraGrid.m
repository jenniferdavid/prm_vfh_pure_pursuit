...
while true
   ...
   % ********************************************************************* 
    ...
    if(row<nrows+1 && row-1>0 && column>0 && column<ncols+1)
        ...
        if(row-1==rowDest && column==columnDest)
            %当前点的上一个点的父亲是当前点
            parent(10*(column-1)+row-1)=10*(column-1)+row; 
            distanceFromStart(node) = tmp+1;
            numExpanded = numExpanded+1; 
            break;
        elseif(distanceFromStart(node)==Inf && visited(node)==0 && map(node)~=2)
            map(node)=4;%on list
            %当前点的上一个点的父亲是当前点
            parent(10*(column-1)+row-1)=10*(column-1)+row;
            distanceFromStart(node) = tmp+1;
    ...
    if(row<nrows+1 && row>0 && column-1>0 && column<ncols+1)
        ...
        if(row==rowDest && column-1==columnDest)
            %当前点的上一个点的父亲是当前点
            parent(10*(column-1-1)+row)=10*(column-1)+row; 
            distanceFromStart(node) = tmp+1;
            numExpanded = numExpanded+1; 
            break;
        elseif(distanceFromStart(node)==Inf && visited(node)==0 && map(node)~=2)
