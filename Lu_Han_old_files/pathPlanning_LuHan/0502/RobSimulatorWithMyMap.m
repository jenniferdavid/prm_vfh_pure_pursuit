classdef RobSimulatorWithMyMap < handle
    %RobSimulatorWithMyMap Simulator for simple differential-drive robot
    ...
    %
    %   OBJ = RobSimulatorWithMyMap creates a simple simulator for a
    ...
    %
    %   OBJ = RobSimulatorWithMyMap(MAPNAME) loads a predefined map
    ...
    %
    %   OBJ = RobSimulatorWithMyMap(MAPNAME,MAPRESOLUTION) loads a
    %   predefined map with MAPNAME and sets the resolution to
    %   MAPRESOLUTION (in cells / meter). By default, the resolution is 2
    %   cells / meter.
    %
    %   OBJ = RobSimulatorWithMyMap(BOG) loads the map defined by the
    %   robotics.BinaryOccupancyGrid object BOG into the simulator.
    %
    %   Notes:
    %   1) RobSimulatorWithMyMap's ROS interface expects MATLAB ROS
    ...
    %
    %   RobSimulatorWithMyMap Published Topics:
    ...
    %
    %   RobSimulatorWithMyMap Subscribed Topics:
    %       /mobile_base/commands/velocity - Velocity commands to the robot
    %
    %   RobSimulatorWithMyMap Services:
    ...
    %   various properties.
    %
    %   RobSimulatorWithMyMap properties:
    %
    ...
    %
    %     % Create Simulator
    %     robotsim = RobSimulatorWithMyMap;
    %
    ...
    properties(Constant, Access = protected)
        ...
        FigureTag = 'RobSimulatorWithMyMap'
    end
    ...
    properties (Access = {?RobSimulatorWithMyMap, ?matlab.unittest.TestCase})
    ...
    methods
        function obj = RobSimulatorWithMyMap(varargin)
            %RobSimulatorWithMyMap Constructor for simulator object
            %   Please check the class definition for details.
            
            obj.MyFigureTag = RobSimulatorWithMyMap.FigureTag;
            ...
            switch nargin
                ...
                case 1
                    % RobSimulatorWithMyMap(MAPNAME) or
                    % RobSimulatorWithMyMap(BOG) syntax
                    map = varargin{1};
                    if ischar(map)
                        % RobSimulatorWithMyMap(MAPNAME) syntax
                        mapResolution = 2;
                        mapName = validatestring(map, fieldnames(exampleMaps), 'RobSimulatorWithMyMap', 'mapName');
                        obj.InternalMap = robotics.BinaryOccupancyGrid(exampleMaps.(mapName), mapResolution);
                    else
                        % RobSimulatorWithMyMap(BOG) syntax
                        validateattributes(map, {'robotics.BinaryOccupancyGrid'}, {'nonempty','scalar'}, ...
                            'RobSimulatorWithMyMap', 'bog');
                        obj.InternalMap = map;
                    end
                case 2
                    % RobSimulatorWithMyMap(MAPNAME, MAPRESOLUTION) syntax
                    mapName = varargin{1};
                    mapResolution = varargin{2};
                    mapName = validatestring(mapName, fieldnames(exampleMaps), 'RobSimulatorWithMyMap', 'mapName');
                    validateattributes(mapResolution, {'double'}, {'nonempty','scalar'}, 'RobSimulatorWithMyMap', 'mapResolution');
                    obj.InternalMap = robotics.BinaryOccupancyGrid(exampleMaps.(mapName), mapResolution);
                otherwise
                    error('RobotSimulator:InvalidInput',...
                        'Invalid number of arguments to RobSimulatorWithMyMap.');
            end
