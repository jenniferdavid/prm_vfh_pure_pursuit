classdef RobotSimulatorWithMyMap < handle
    %RobotSimulatorWithMyMap Simulator for simple differential-drive robot
    ...
    %
    %   OBJ = RobotSimulatorWithMyMap creates a simple simulator for a
    ...
    %
    %   OBJ = RobotSimulatorWithMyMap(MAPNAME) loads a predefined map
    ...
    %
    %   OBJ = RobotSimulatorWithMyMap(MAPNAME,MAPRESOLUTION) loads a
    %   predefined map with MAPNAME and sets the resolution to
    %   MAPRESOLUTION (in cells / meter). By default, the resolution is 2
    %   cells / meter.
    %
    %   OBJ = RobotSimulatorWithMyMap(BOG) loads the map defined by the
    %   robotics.BinaryOccupancyGrid object BOG into the simulator.
    %
    %   Notes:
    %   1) RobotSimulatorWithMyMap's ROS interface expects MATLAB ROS
    ...
    %
    %   RobotSimulatorWithMyMap Published Topics:
    ...
    %
    %   RobotSimulatorWithMyMap Subscribed Topics:
    %       /mobile_base/commands/velocity - Velocity commands to the robot
    %
    %   RobotSimulatorWithMyMap Services:
    ...
    %   various properties.
    %
    %   RobotSimulatorWithMyMap properties:
    %
    ...
    %
    %   RobotSimulatorWithMyMap methods:
    %
    ...
    %
    %     % Create Simulator
    %     robotsim = RobotSimulatorWithMyMap;
    %
    ...
    properties(Constant, Access = protected)
        ...
        FigureTag = 'RobotSimulatorWithMyMap'
    end
    ...
    properties (Access = {?RobotSimulatorWithMyMap, ?matlab.unittest.TestCase})
    ...
    methods
        function obj = RobotSimulatorWithMyMap(varargin)
            %RobotSimulatorWithMyMap Constructor for simulator object
            %   Please check the class definition for details.
            
            obj.MyFigureTag = RobotSimulatorWithMyMap.FigureTag;
            ...
            switch nargin
                case 0
                    % RobotSimulatorWithMyMap() syntax
                    % Use default values
                    mapName = 'emptyMap';
                    mapResolution = 2;
                ...
                case 1
                    % RobotSimulatorWithMyMap(MAPNAME) or
                    % RobotSimulatorWithMyMap(BOG) syntax
                    map = varargin{1};
                    if ischar(map)
                        % RobotSimulatorWithMyMap(MAPNAME) syntax
                        mapResolution = 2;
                        mapName = validatestring(map, fieldnames(exampleMaps), 'RobotSimulatorWithMyMap', 'mapName');
                        obj.InternalMap = robotics.BinaryOccupancyGrid(exampleMaps.(mapName), mapResolution);
                    else
                        % RobotSimulatorWithMyMap(BOG) syntax
                        validateattributes(map, {'robotics.BinaryOccupancyGrid'}, {'nonempty','scalar'}, ...
                            'RobotSimulatorWithMyMap', 'bog');
                        obj.InternalMap = map;
                    end
                case 2
                    % RobotSimulatorWithMyMap(MAPNAME, MAPRESOLUTION) syntax
                    mapName = varargin{1};
                    mapResolution = varargin{2};
                    mapName = validatestring(mapName, fieldnames(exampleMaps), 'RobotSimulatorWithMyMap', 'mapName');
                    validateattributes(mapResolution, {'double'}, {'nonempty','scalar'}, 'RobotSimulatorWithMyMap', 'mapResolution');
                    obj.InternalMap = robotics.BinaryOccupancyGrid(exampleMaps.(mapName), mapResolution);
                otherwise
                    error('RobotSimulator:InvalidInput',...
                        'Invalid number of arguments to RobotSimulatorWithMyMap.');
            end
